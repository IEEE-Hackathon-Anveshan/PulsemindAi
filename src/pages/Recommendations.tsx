  ...
  return (
    ...
    >
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        {readinessData && readinessData.currentPhase !== 'full-access' && (
          <div className="bg-gradient-to-r from-yellow-500/20 to-orange-500/20 border border-yellow-500/50 rounded-xl p-6 mb-8">
            <div className="flex items-start space-x-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-white/80 mb-3">
                  {readinessData.currentPhase === 'community-readonly'
                    ? 'You can view community content. Keep engaging to unlock posting and chat!'
                    : 'Complete your wellness journey to unlock safe community access.'}
                </p>
                <div className="flex items-center space-x-3">
                  ...
                  <span className="text-yellow-400 font-bold">{readinessData.readinessScore}%</span>
                </div>
                <p className="text-white/60 text-sm mt-2">Next: {readinessData.nextMilestone}</p>
              </div>
            </div>
          </div>
        )}
        ...
        <div className="flex justify-between items-center mb-8">
          ...
          <div className="flex space-x-4">
            <button
              ...
              disabled={readinessData?.currentPhase !== 'full-access'}
              className={`px-6 py-2 rounded-full transition-all flex items-center ${
                readinessData?.currentPhase === 'full-access'
                  ? 'bg-stress-yellow text-stress-dark hover:bg-opacity-90 cursor-pointer'
                  : 'bg-gray-600 text-gray-400 cursor-not-allowed opacity-60'
              }`}
              title={readinessData?.currentPhase !== 'full-access' ? 'Unlock full access to chat' : ''}
            >
            ...
            <Link
              ...
              className={`px-6 py-2 rounded-full transition-all flex items-center ${
                readinessData?.currentPhase === 'full-access'
                  ? 'bg-stress-yellow text-stress-dark hover:bg-opacity-90'
                  : 'bg-gray-600 text-gray-400 pointer-events-none opacity-60'
              }`}
            >
                ...
                <div key={event._id} className="bg-gray-900 rounded-lg p-6 border border-yellow-500/20">
                  ...
                  {user && event.status === 'open' && event.currentParticipants < event.maxParticipants && (
                    <button
                      ...
                      className="mt-4 w-full bg-yellow-500 text-black px-4 py-2 rounded hover:bg-yellow-400 transition-all"
                    >
                      Join Event
                    </button>
                  )}
