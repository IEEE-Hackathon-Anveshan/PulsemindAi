...
const MoodAssessment: React.FC = () => {
  ...
  const generateNextQuestion = async (previousAnswers: Answer[]) => {
    ...
    try {
      ...
      // Use gemini-2.5-flash (same as chat feature)
...
Generate the next question:`;
  ...
  useEffect(() => {
    ...
    if (needsMoreQuestions && currentQuestionExists && nextQuestionMissing && hasAnswers && !isGenerating) {
      // Generate next question in background
      generateNextQuestion(answers);
    }
  }, [answers.length, currentQuestion, allQuestions.length]);

  // If we landed on a question that doesn't exist, generate it immediately
  useEffect(() => {
    if (!allQuestions[currentQuestion] && !isGenerating && answers.length >= 4) {
      generateNextQuestion(answers);
    }
  }, [currentQuestion]);
      ...
      <div className="min-h-screen flex items-center justify-center p-4">
        <div className="bg-gray-500/10 backdrop-blur-3xl p-8 rounded-2xl shadow-2xl border border-white/20 max-w-2xl w-full">
          ...
          <div className="space-y-8">
            ...
            {analysisResult && (
              <div className="bg-white/5 rounded-xl p-6 border border-white/10">
                <p className="text-yellow-400 font-semibold mb-2">Your Current State</p>
            ...
            {analysisResult && (
              <div className="bg-white/5 rounded-xl p-6 border border-white/10">
                <p className="text-yellow-400 font-semibold mb-3">Personalized Recommendations</p>
                  ...
                  {analysisResult.therapies.map((therapy, index) => {
                    ...
                    return (
                      <button
                        ...
                        className="bg-yellow-500/10 border border-yellow-500/30 rounded-lg p-4 text-yellow-400 font-medium text-center hover:bg-yellow-500/20 hover:border-yellow-500/50 hover:scale-105 transition-all duration-300 cursor-pointer"
                      >
                        {therapy}
                      </button>
                    );
            ...
            <div className="flex justify-center space-x-4">
              ...
              <button
                ...
                className="bg-yellow-500 text-black px-6 py-3 rounded-full hover:bg-yellow-400 transition-all font-semibold flex items-center space-x-2"
              >
        ...
        {allQuestions[currentQuestion] && !isGenerating ? (
              ...
              {[1, 2, 3, 4, 5].map((value) => (
                <button
                  ...
                  className="group relative p-6 rounded-xl border-2 transition-all duration-300 hover:scale-105 hover:-translate-y-1
                    bg-white/5 hover:bg-white/15 border-white/20 hover:border-yellow-400"
                >
          ...
          <div className="text-center py-8">
            ...
            <button
              ...
              className="mt-4 bg-yellow-500 text-black px-6 py-3 rounded-full hover:bg-yellow-400 transition-all font-semibold"
            >
              Generate Question
            </button>
          </div>
